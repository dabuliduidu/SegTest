...
def objectInsertion(labelName,imageName,obj,insertpoint,times):
    count = len(os.listdir('data/segnet_guided_'+str(times)+'_image/'))
    newImagename = 'data/segnet_guided_'+str(times)+'_image/'+str(count)+'.png'
    newLabelname = 'data/segnet_guided_'+str(times)+'_label/'+str(count)+'.png'
    ...
    objimg = cv2.resize(objimg,(int(objimg.shape[1]/8),int(objimg.shape[0]/8)))
    # objimg = cv2.resize(objimg,(int(objimg.shape[1]*scale),int(objimg.shape[0]*scale)))
    centroid = (int(obj[6][0]/8),int(obj[6][1]/8))
    objsize = obj[1]
    ...
    f = codecs.open('log_segnet_guided_'+str(times)+'.txt','a','utf-8')
    f.write(str(log)+',')
    f.close()
...
def ruleGenerate(augmentList,times):
    for augmentation in augmentList:
        ...
        label = image.replace('backgrounds','segnet_backseg')
