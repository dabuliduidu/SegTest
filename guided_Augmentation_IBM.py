...
def objectInsertion(labelName,imageName,obj,insertpoint,times):
    count = len(os.listdir('data/IBM_guided_'+str(times)+'_image/'))
    newImagename = 'data/IBM_guided_'+str(times)+'_image/'+str(count)+'.png'
    newLabelname = 'data/IBM_guided_'+str(times)+'_label/'+str(count)+'.png'
    ...
    f = codecs.open('log_IBM_guided_'+str(times)+'.txt','a','utf-8')
    f.write(str(log)+',')
    f.close()
...
def ruleGenerate(augmentList,times):
    for augmentation in augmentList:
        ...
        label = image.replace('backgrounds','IBM_backseg')
