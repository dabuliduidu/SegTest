...
def objectInsertion(labelName,imageName,obj,insertpoint,times):
    count = len(os.listdir('data/hrnet_guided_'+str(times)+'_image/'))
    newImagename = 'data/hrnet_guided_'+str(times)+'_image/'+str(count)+'.png'
    newLabelname = 'data/hrnet_guided_'+str(times)+'_label/'+str(count)+'.png'
    ...
    f = codecs.open('log_hrnet_guided_'+str(times)+'.txt','a','utf-8')
    f.write(str(log)+',')
    f.close()
