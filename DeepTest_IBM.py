...
def translation(imgName,lblName,tx,ty,times):
    ...
    list = os.listdir('data/IBM_deeptest_'+str(times)+'_image/')
    count = len(list)
    newimgName = 'data/IBM_deeptest_'+str(times)+'_image/'+ str(count) + '.png'
...
def shear(imgName,lblName,sx,sy,times):
    ...
    lbl = cv2.imread(lblName)
    img1 = cv2.resize(img1,(lbl.shape[1],lbl.shape[0]))
    list = os.listdir('data_deeptest_new/IBM_deeptest_'+str(times)+'_image/')
    count = len(list)
    newimgName = 'data_deeptest_new/IBM_deeptest_'+str(times)+'_image/'+ str(count) + '.png'
    newlblName = newimgName.replace('image', 'label')
    # 2 仿射变换
    rows, cols = img1.shape[:2]
    # 2.1 创建变换矩阵
...
if __name__ == '__main__':
    ...
    for times in [1,2,3,4,5]:
        for img in imgs:
            ...
            lblName = imgName.replace('backgrounds', 'IBM_backseg')
