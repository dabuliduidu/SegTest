...
def objectInsertion(labelName,imageName,obj,insertpoint,times):
    count = len(os.listdir('data/segnet_random_'+str(times)+'_image/'))
    newImagename = 'data/segnet_random_'+str(times)+'_image/'+str(count)+'.png'
    newLabelname = 'data/segnet_random_'+str(times)+'_label/'+str(count)+'.png'
    ...
    f = codecs.open('log_segnet_random_'+str(times)+'.txt','a','utf-8')
    f.write(str(log)+',')
    f.close()
