...
def objectInsertion(labelName,imageName,obj,insertpoint,times):
    count = len(os.listdir('data/IBM_random_'+str(times)+'_image/'))
    newImagename = 'data/IBM_random_'+str(times)+'_image/'+str(count)+'.png'
    newLabelname = 'data/IBM_random_'+str(times)+'_label/'+str(count)+'.png'
    ...
    f = codecs.open('log_IBM_random_'+str(times)+'.txt','a','utf-8')
    f.write(str(log)+',')
    f.close()
